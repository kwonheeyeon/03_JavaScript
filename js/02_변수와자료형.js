...
number2 = 20; // number2라는 메모리 공간에 20을 집어넣음
...
function letCheck() {
  ...
  // Identifier 'name' has already been declare
  // -> name 변수가 이미 선언됨
...
// const로 선언된 변수에 값을 대입해도 변경할 수 없다(새로운 값 대입 불가)
function constCheck() {
  ...
  // 02_변수와자료형.js:84 -> 84번째 줄에서 오류 발생
  
  //Uncaught TypeError: Assignment to constant variable.at constCheck
  // -> 상수 변수에 값이 할당됨
  // --> 상수는 값을 다시 대입(할당)할 수 없어서 오류 발생

  // 해결방법 : const를 let으로 변경
...
function scopeTest() {
  ...
  // ReferenceError: num4 is not defined -> num4가 정의되지 않음
}
...
function scopeTest2() {
  ...
  if(1 == 1) {
    ...
    // 함수 지역변수 temp와 같은 이름의 if 지역변수 temp 선언
    const temp = 456;
...
function typeCheck() {
  ...
  /* undefiend : 정의되지 않은 변수 / 값 대입 X 또는 존재 X 변수 */
  let undef; 
  console.log("under :", undef, typeof undef);
