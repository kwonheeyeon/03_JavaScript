...
let currentInterval; // START 버튼을 눌렀을 때
...
function output(){
  ...
  // -> 1분 증가
  if(timeList[0].innerText !== minute){
    timeList[0].innerText = minute;
  }

  // second에 저장된 값과 화면에 출력된 "초"가 다른경우
  // -> 1초 증가
  if(timeList[1].innerText !== second){
...
resetBtn.addEventListener("click", ()=>{
  /* [window.]clearInterval(serInterval()이 저장된 변수)
    - 전달인자에 작성된 setInterval()을 멈춤(지움)
  */
  clearInterval(currentInterval);

  // 화면에 출력되는 시간을 00:00:00으로 초기화
  count = 0;
  output();
