...
function ageCheck2(){
  ...
  // 가장 넓은 범위부터 제외
  // 0 미만 또는 120 초과
  if(value < 0 || value > 120)  result = "잘못입력하셨습니다";
...
function check4(){
  ...
  if(input4.value.length === 0){ // - 문자열.length == 문자열의 길이를 반환
    alert("값을 입력해주세요");
  }else{ // 값 입력 O
    // 여기서는 0도 양수로 취급
    if(value >= 0){ // 양수
      if(value % 3 === 0){ // 3의 배수가 맞으면  : YES
        alert("YES");
      }else{ // 3의 배수가 아니면  : NO
        alert("NO");
      }
...
function check5(){
 ...
 // 1) A와 B 중 하나라도 입력되지 않으면 -> 입력되지 않은 값이 존재합니다
 ...
 // 3) 1,2번 조건이 모두 false이면 배수 판별 진행 -> 배수 맞음 / 배수 아님
 if(valueA % valueB === 0){
  alert("배수맞음");
  return;
 }
...
function calc(op){
  ...
  */
  ...
  // 아이디가 calcResult인 요소를 얻어와 내용으로 result를 대입
